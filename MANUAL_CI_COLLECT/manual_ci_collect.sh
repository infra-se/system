...
then
	...
	FUNCT_CATEGORY=CI_SHEET_02
		...
		do
			...
			echo "${FUNCT_CATEGORY}|${HOSTNAME}|${CHECK_PLATFORM}|${MOUNT_POINT}|${CHECK_PERM_ALL}|${DISK_DEVICE}|${FS_TYPE}|${DISK_SIZE_GB}|${DISK_TYPE}|${USED_SIZE_MB}|${CUMULATION_DATE}|${DAILY_CUMULATION}"
	...
	FUNCT_CATEGORY=CI_SHEET_03
		...
		do
			...
			echo "${FUNCT_CATEGORY}|${HOSTNAME}|${ACCOUNT}|${ACCOUNT_UID}|${ACCOUNT_GID}|${ACCOUNT_GIDS}|${HOME_DIR}|${SHELL_PATH}"
		done
	}
	...
	FUNCT_CATEGORY=CI_SHEET_04
		...
		do
			...
			ARRAY_PROCESS_INFO=("${ARRAY_PROCESS_INFO[@]}" "`echo "${FUNCT_CATEGORY}|${HOSTNAME}|${PROCESS_OWN}|${PROCESS_CMD}"`")	
		done < ${TMP_PROC_LIST}
	...
	FUNCT_CATEGORY=CI_SHEET_05
		...
		do
			...
			echo "${FUNCT_CATEGORY}|${HOSTNAME}|${CHECK_PLATFORM}|${LIST}|${MAC_ADDRESS}|${IP_ADDRESS}|${ETH_TYPE}|${CHECK_LINK_STAT}"
		done
	}
	...
	FUNCT_CATEGORY=CI_SHEET_06
		...
		do
			...
			ARRAY_SESSION_INFO=("${ARRAY_SESSION_INFO[@]}" "`echo "${FUNCT_CATEGORY}|${HOSTNAME}|${PROTOCAL}|${SOURCE}|${DESTINATION}|${SESSION_STAT}|${PROCESS_INFO}"`")	
		done < ${TMP_PROC_LIST}
	...
	FUNCT_CI_COLLECT_01 | sed 's#^ *##g' | sed 's#CI_SHEET_01|##g' | tee ${CI_LOG_01}
	FUNCT_CI_COLLECT_02 | sed 's#^ *##g' | sed 's#CI_SHEET_02|##g' | tee ${CI_LOG_02}
	FUNCT_CI_COLLECT_03 | sed 's#^ *##g' | sed 's#CI_SHEET_03|##g' | tee ${CI_LOG_03}
	FUNCT_CI_COLLECT_04 | sed 's#^ *##g' | sed 's#CI_SHEET_04|##g' | tee ${CI_LOG_04}
